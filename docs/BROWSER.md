...
브라우저는 pane 안의 탭으로 열리며 Windows의 WebView2를 사용한다. 터미널 셸이나 별도 브라우저 데몬을 만들지 않는다. pane 헤더의 `◎` 버튼으로 열고 주소창에 HTTP(S) URL을 입력한다. 워크스페이스가 없는 첫 화면에서도 **New browser tab**으로 시작할 수 있다. 브라우저 자체는 WSL 없이 사용할 수 있다.
...
WSL이 없으면 `%AppData%\app.mast.desktop\settings.json`을 직접 편집한다.
...
ON이어도 페이지를 표시하거나 에이전트가 페이지 작업을 요청하기 전에는 웹뷰를 만들지 않는다. 숨긴 탭은 WebView2 휴면을 요청하고, 작업할 때 깨운다. 휴면은 런타임이 거부할 수 있으며 메모리 반환을 보장하지 않는다. 닫으면 웹뷰와 페이지 버퍼를 해제한다. 입력 내용 손실을 피하기 위해 숨긴 페이지를 자동 폐기하지 않는다.

쿠키와 웹 저장소는 `%AppData%\app.mast.desktop\browser\workspace-<id>`에 워크스페이스별로 분리한다. 같은 워크스페이스의 탭은 로그인 상태를 공유한다. 탭을 닫아도 프로필은 보존된다. 앱 재시작 시 URL과 배치만 복원하며 페이지의 미저장 입력·탐색 기록까지 복원하지는 않는다.

브라우저 웹뷰가 있는 동안 메모리 임계치만을 근거로 한 Mast UI 자동 리셋을 억제한다. 전체 프로세스 트리 메모리 측정은 계속 유지한다. 모든 브라우저 웹뷰를 닫으면 기존 메모리 리셋 정책이 다시 적용된다.

## 에이전트가 같은 탭 확인하기

Mast 안의 WSL 터미널에서 `mast browser`를 사용한다. 기존 OSC 요청·응답 경로를 이용하며 별도 포트나 MCP 서버를 열지 않는다. 호출한 터미널과 같은 워크스페이스의 탭만 제어한다. 결과는 JSON이며 실패 시 종료 코드는 1이다.
...
스크린샷은 현재 viewport의 PNG이며 최대 16메가픽셀이다. `--output`을 생략하면 WSL `/tmp/mast-browser-*.png`에 저장하고 경로를 반환한다. 기존 파일을 덮어쓰지 않는다. 에이전트가 해당 경로를 읽어 확인한 뒤 필요 없는 캡처는 직접 삭제한다. 요청은 32 KiB, 응답은 24 MiB로 제한한다. snapshot은 최대 400개 요소와 본문 40,000자, 콘솔·오류 버퍼는 각각 최근 100개 항목을 보관한다.

## 지원 범위

- 주소 입력과 탐색은 HTTP(S)만 허용한다. 자격 증명을 URL에 넣거나 `file:`, `javascript:` 등의 스킴을 사용하는 요청은 거부한다.
- 페이지 팝업은 같은 pane의 브라우저 탭으로 전환한다. 다운로드와 카메라·마이크 등 웹 권한 요청은 거부하고 상태에 이유를 표시한다. 필요한 경우 주소창 옆 외부 브라우저 버튼을 사용한다.
