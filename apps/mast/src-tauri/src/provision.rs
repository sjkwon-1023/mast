...
/// 스킬 히어독에도 걸린다: 원본은 `scripts/wsl/skills/mast-send/SKILL.md` 와
/// `scripts/wsl/skills/mast/SKILL.md` 다 (hook-example.test.ts·provision-setup.test.ts 가
/// 바이트 동일성을 검사한다).
///
...
# mast provisioning — 앱이 첫 실행 때 배포판마다 한 번 `wsl.exe [-d <distro>] -- bash -s`로
# 흘려 넣는다. 에이전트 알림 스크립트, 훅 디스패처, mast CLI를 설치하고,
# scripts/wsl/claude-hook-example.md가 설명하는 Claude Code·Codex·Antigravity CLI 훅을 연결하며,
# 에이전트 스킬(scripts/wsl/skills/)과 전역 OpenCode 플러그인을 설치한다.
#
# 여기서 stdin을 읽으면 안 된다: 그 스트림이 이 스크립트 자신이다.
...
# mast 사용법 스킬 본문: mast 탭 안에서 에이전트가 할 수 있는 일 — CLI, pane-send 채널,
