...
// **라이브로** 건다.
//
// 종전에는 이 두 값이 xterm 옵션에만 들어가서(features/terminal/view.ts
// applyTerminalSettings) 텍스트 뷰어의 행·폴더 목록·마크다운 code 는 app/styles.css 에
...
// app/styles.css 가 `var(--viewer-font-family, monospace)` 꼴로 읽는다.
//
...
//   읽는지는 app/styles.css 가 정본이고 features/viewers/viewer-font.test.ts 가 그 목록을 잠근다.
...
// 커지는데, 텍스트 뷰어 가상 스크롤의 행 격자(features/viewers/text/view.ts 의 `lineHeight`)는 뷰
...
// - **클램프 6~72 정수** — 범위의 정본은 shared/font-size.ts(→ 백엔드 FONT_SIZE_RANGE)다.
// - **키 1개가 두 표면을 같은 스텝으로** 움직인다 (터미널 + 뷰어). 표면별 독립
//   줌은 만들지 않는다 — 두 함수를 같이 부르는 자리는 app/main.ts runNavAction 한
//   곳이고, 키 판정은 shared/keys.ts 가로채기 표가 정본이다. 두 표면의 **기준값**은
 ...
 *
 *  **app/styles.css 미러 계약**: `var(--viewer-font-size, 12px)` 를 쓰는 셀렉터들의
 ...
 *  features/viewers/viewer-font.test.ts 가 잠근다) 화면은 미설정과 동일하다. */
