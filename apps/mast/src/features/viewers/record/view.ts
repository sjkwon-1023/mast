...
//
// 수명은 뷰어의 것이다 (features/viewers/viewer-view.ts 의 TerminalRecordKind): 활성 탭일 때만
...
// 한 스텝에 같이 움직여야 한다. 뷰어 글꼴(features/viewers/viewer-font.ts) 쪽이 아닌 이유가 그것
// 이다: 같은 pane 에서 살아 있는 셸과 그 셸의 마지막 화면이 다른 크기로 보이면
// 안 된다.
//
// 리사이즈는 **자체 ResizeObserver** 로 받는다 (뷰어 관례 — features/viewers/text/view.ts 와 같다).
...
import { readTabRecord } from "../../../infrastructure/backend";
...
export class RecordView implements ViewerView, TerminalFontTarget {
  ...
  constructor(parent: HTMLElement, tab: TabId) {
    ...
    // 조용히 죽으면 안 된다. 판정·동작은 터미널 뷰와 공유한다 (shared/keys.ts 정본 표).
