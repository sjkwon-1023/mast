...
function settings(fontFamily: string | null, fontSize: number | null): UiSettings {
  return { fontFamily, fontSize, highlightLanguages: null, log: null, remote: null };
}
...
// 결과로 행 격자가 실제로 다시 앉는지는 뷰가 필요하므로 features/viewers/text/view.test.ts 몫이다.
describe("뷰어 줌", () => {
  /** 레지스트리 계약만 보면 되므로 DOM 이 필요 없다 (실제 격자 재계산은
   *  features/viewers/text/view.test.ts 가 본다). */
  ...
  // 순서에 무관하게 만든다 (features/viewers/text/view.test.ts 의 같은 관례).
  afterEach(() => {
    applyViewerFontSettings(settings(null, null));
  });
