#!/usr/bin/env bash
# Codex 훅 진입점. 이벤트 구분은 stdin 의 hook_event_name 이 하므로 모든 이벤트가 이 명령 하나를 쓴다.
#
# 이 명령 문자열은 Codex trust hash 의 입력이라 인자를 받지 않는다. 스크립트 내용이 바뀌어도
# 재신뢰가 필요 없다. 결과와 무관하게 exit 0 이고 stdout 에는 아무것도 쓰지 않는다. Codex 는
# exit 2 를 block/deny 로, stdout 을 훅 JSON 출력으로 읽는다. 그래서 인터프리터를 exec 하지
# 않는다 — exec 하면 python 크래시 코드가 그대로 Codex 에 전달된다.
MODE=codex
